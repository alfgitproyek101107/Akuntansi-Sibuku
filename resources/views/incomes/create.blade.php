    ...
    <header class="dashboard-header">
        <div class="header-container">
            <div class="header-left">
                ...
                <h1 class="page-title">Tambah Pemasukan Baru</h1>
                <p class="page-subtitle">Catat pemasukan baru untuk meningkatkan arus kas Anda</p>
            </div>
            ...
            <div class="form-section">
                <div class="form-container">
                    <div class="form-header">
                        ...
                        <h2 class="form-title">Form Pemasukan Baru</h2>
                        <p class="form-subtitle">Masukkan detail pemasukan dengan lengkap dan akurat</p>
                    </div>

                    <div class="form-body">
                        <form method="POST" action="{{ route('incomes.store') }}" id="incomeForm" onsubmit="return validateForm()">
                            @csrf
                            ...
                            <div class="form-group">
                                ...
                                <div class="mode-toggle">
                                    ...
                                    <div class="mode-option {{ request('input_mode', 'simple') === 'service' ? 'active' : '' }}"
                                         onclick="selectInputMode('service')">
                                        <input type="radio" name="input_mode" value="service"
                                               {{ request('input_mode', 'simple') === 'service' ? 'checked' : '' }}
                                               style="display: none;">
                                        <div class="mode-icon">
                                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                ...
                                <div class="product-selection">
                                    <div class="product-grid">
                                        @foreach($products ?? [] as $product)
                                            <div class="product-card" onclick="addToCart({{ $product->id }}, {{ json_encode($product->name) }}, {{ $product->price }}, {{ $product->stock_quantity }})">
                            ...
                            <div class="form-group" id="serviceSelectionGroup" style="{{ request('input_mode', 'simple') === 'service' ? '' : 'display: none;' }}">
                                <label class="form-label">
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                        <circle cx="9" cy="21" r="1"></circle>
                                        <circle cx="20" cy="21" r="1"></circle>
                                ...
                                <div class="service-selection">
                                    <div class="service-grid">
                                        @foreach($services ?? [] as $service)
                                            <div class="service-card" onclick="addServiceToCart({{ $service->id }}, {{ json_encode($service->name) }}, {{ $service->price }})">
                                ...
                                <div class="cart-container">
                                    ...
                                    <div class="cart-summary">
                                        <div class="summary-row total-row">
                                            <span class="summary-label">Total:</span>
                                            <span class="summary-amount" id="serviceCartTotalAmount">Rp0</span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <!-- Service Amount Display (shown when input_mode is service) -->
                            <div class="form-group" id="serviceAmountDisplayGroup" style="{{ request('input_mode', 'simple') === 'service' ? '' : 'display: none;' }}">
                                <div class="amount-display">
                                    <div class="amount-label">Jumlah Pemasukan dari Layanan:</div>
                                    <div class="amount-value">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                            <line x1="12" y1="5" x2="12" y2="19"></line>
                                            <line x1="5" y1="12" x2="19" y2="12"></line>
                                        </svg>
                                        <span id="serviceAmount">Rp0</span>
                                    </div>
                                    <input type="hidden" name="calculated_amount" id="serviceCalculatedAmount" value="{{ old('calculated_amount', 0) }}" readonly>
                                </div>
                            </div>
                    ...
                    <div class="tips-content">
                        <div class="tip-item">
                            ...
                            <div class="tip-text">
                                ...
                                <p>Selalu tambahkan keterangan yang jelas untuk setiap pemasukan agar mudah dilacak di masa depan.</p>
                            </div>
                        </div>
                        ...
                        <div class="tip-item">
                            ...
                            <div class="tip-text">
                                ...
                                <p>Pastikan tanggal pemasukan sesuai dengan saat uang benar-benar diterima.</p>
                            </div>
                        </div>
...
function selectInputMode(mode) {
    ...
    const serviceAmountGroup = document.getElementById('serviceAmountDisplayGroup');
    ...
    if (mode === 'simple') {
        ...
        document.querySelector('input[name="service_id"]').required = false;
        ...
        // Make amount not required, service not required
        ...
        // Make amount not required, service not required (handled by cart validation)
        document.querySelector('input[name="amount"]').required = false;
    ...
    if (mode === 'service') {
        ...
        document.querySelector('input[name="service_id"]').checked = false;
        selectService(null, null); // Reset service amount
    }
}
...
function addServiceToCart(serviceId, serviceName, price) {
    console.log('addServiceToCart called:', { serviceId, serviceName, price });
...
function updateServiceCartDisplay() {
    ...
    serviceCartItems.forEach((item, index) => {
        ...
        serviceCartItemsContainer.innerHTML += `
            ...
            <input type="hidden" name="services[${index}][subtotal]" value="${subtotal}">
        `;
    });

    // Update summary
    serviceCartTotalAmount.textContent = 'Rp' + totalAmount.toLocaleString('id-ID');
}
